...
<form class="layui-form" action="">
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input value="{{$result['keyword']}}" type="text" id="keyword" name="keyword" required  lay-verify="required" placeholder="请输入关键词" autocomplete="off" class="layui-input">
        </div>
    </div>
    ...
    <div class="layui-form-item" id="cons">
        ...
        <div class="layui-input-block">
            <input class="type" type="radio" name="type" value="1" title="文字"  lay-filter="jiedian" {{$result['wenzi']}} >
            <input  class="type" type="radio" name="type" value="2" title="图文"  lay-filter="jiedian" {{$result['tuwen']}}>
        </div>
    </div>
    ...
    <div class="layui-form-item layui-form-text" id="text">
        ...
        <div class="layui-input-block">
            <textarea name="reply_content" id="reply_content" placeholder="请输入内容" class="layui-textarea">{{$result['reply_content']}}</textarea>
        </div>
    </div>
    ...
    <div id="image">
        <div class="news" style="">
            <div class="imgT-penl" style="margin-left:100px;">
                <div class="wx-penl">
                    <div class=" item">
                        <input type="text"  name="title" value="{{$result['title']}}" class="layui-input J_Title" placeholder="图文标题">
                    </div>
                    <div class="img-box">
                        <input type="hidden" name="image" value="">
                        <img class="J_Image" style="height: 30%" id="img" src="{{$result['img']}}" alt="" width="100%">
                        <div class="zz">
                            <a class="layui-btn" id="test1">更换图片</a>
                        </div>
                    </div>
                    <div style="margin-bottom: 15px;" class=" item">
                        <input type="text"  name="description" id="desc" class="layui-input" placeholder="输入图文简介25字内" value="{{$result['desc']}}">
                    </div>

                    <div class="item">
                        <input type="text"  value="{{$result['url']}}" name="url" id="url" class="layui-input J_Title" placeholder="图片地址">
                    </div>
    ...
    <input type="hidden" id="ids" value="{{$result['id']}}">
...
<script>
    ...
    layui.use(['form','upload'], function(){
        ...
        form.on('radio(jiedian)', function(data){  //监听
            if( data.value == 1){  //文字
        ...
        form.on('submit(*)', function (data) {
            ...
            $.ajax({
                type: "POST",//规定传输方式
                url: "/administrator/wechat_reply_update",//提交URL
                headers: {
