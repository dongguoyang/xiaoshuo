...
trait CollectTrait
{
     ...
     * 带cookie 的 curl 抓取
     */
    private function cookieCurl($url,$option = [], $params = [], $type = 'url', $re = 0,$header=''){
        try {
            return $this->doCookieCurl($url,$option, $params, $type,$header);
        } catch (\Exception $e) {
            dump(
                date('Y-m-d H:i:s'),
                $e->getMessage(),
            ...
            if ($re < 3) {
                $re++;
                sleep($re * 4);
                return $this->cookieCurl($url, $option, $params, $type, $re,$header);
            }
            throw new \Exception($e->getMessage(), $e->getCode());
        }
    }
     ...
     * 执行带cookie 的 curl 抓取
     */
    private function doCookieCurl($url, $option = [], $params = [], $type = 'url',$header=''){
        ...
        $header_arg = array();
        $header_arg[] = 'Accept: image/gif, image/jpeg, image/pjpeg, image/pjpeg, application/x-ms-application, application/x-ms-xbap, application/vnd.ms-xpsdocument, application/xaml+xml, */*';
        $header_arg[] = 'Connection: Keep-Alive';
        $header_arg[] = 'Accept-Language: zh-cn';
        $header_arg[] = 'Cache-Control: no-cache';
        $header_arg[] = $header;
        ...
        $option[CURLOPT_HTTPHEADER]=$header_arg;
