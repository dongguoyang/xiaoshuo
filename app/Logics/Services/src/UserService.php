...
//        if ($sess['id'] && !strpos(request()->getHost(), 'idnqjrn.cn')) {
...
//        }
        if (!isset($_GET['code']) || !$_GET['code']) {
            ...
            if (isset($state['o'])) {
                ...
                $wechat = $this->payWechats->initWechat(1, true,$state['c']);
                ...
                $data = $this->cacheAuth1Info($auth1_info);
                if (!isset($data['state']) || !isset($data['userinfo'])) {
                    throw new \Exception('小说公众号用户微信信息获取失败！', 2000);
                }
                $wechat_userinfo                = $data['userinfo'];
                $wechat_userinfo['pay_openid']  = $auth1_info;
                $wechat_userinfo['pay_wechat_up_num']  = 0;
                $user = $this->register('h5wechat', $wechat_userinfo, $state); // 执行注册
                return $this->DoLogin($user, $state);
            }
        }
    ...
    private function auth1Info($state) {
        ...
        if ($user) {
            /*if (!empty(trim($user['pay_openid']))) {
                ...
                $paywechat = $this->payWechats->initWechat(1, true,$state['c']);
     ...
     * 头条信息上报
     */
    public function PutTouInfo($type = 0,$user_id){
            $user_info = $this->users->find($user_id, $this->users->model->fields);
            //通过ip和user_agent进行匹配
            //获取ip和user_agent
            $ip = request()->getClientIp();
            $user_agent = request()->userAgent();
            $platform = getOS($user_agent);
            $ToutiaoUser = new ToutiaoUser();
            $info = $ToutiaoUser->where('ip',$ip)
                //->where('os',$platform['os_version'])
                ->where('machine_type',$platform['os_platform'])
                ->orderBy('updated_at','desc')->first();            
            if($type == 1){  //关注
                if($user_info['source'] == 0){
                    if($info){
                        //判断有没有过一小时的时间间隔
                        if($info['updated_at']+3600 >time()){
                            //更新用户信息
                            $this->users->update(['source' => $info['id']], $user_id);
                            //数据上报https://ad.toutiao.com/track/activate/?link=__LINK__&source=__SOURCE__&conv_time=__CONVTIME__&event_type=__EVENT_TYPE__
                            $url = 'https://ad.toutiao.com/track/activate/?link='.urlencode($info['url']).'&event_type=0';
                            CallCurl($url);
                        }
                    }
                }
                ...
                //判断有没有过一小时的时间间隔
                $info = $ToutiaoUser->where('id',$user_info['source'])->first();
                if($info['updated_at']+86400 >time()){
                    //更新用户信息
                    $this->users->update(['source' => $info['id']], $user_id);
                    //数据上报https://ad.toutiao.com/track/activate/?link=__LINK__&source=__SOURCE__&conv_time=__CONVTIME__&event_type=__EVENT_TYPE__
                    $url = 'https://ad.toutiao.com/track/activate/?link='.urlencode($info['url']).'&event_type=2';
                    CallCurl($url);
                }
            }
