...
class CommonService extends Service {
    ...
    public function WechatInfo() {
        ...
        [
            "app_name"=>"想采网络",
            "img"=>"https://xiaoshuo-hongbao.oss-cn-beijing.aliyuncs.com/images/ebc6bbba3849387f89d54853e9c51836.jpg",
            "appid"=>"wx5cc3e04beaeb06d5",
            "customer_service"=>"https://xiaoshuo-hongbao.oss-cn-beijing.aliyuncs.com/commonset/img/6a0c1e22e6e257139256cf40b224aab6.png"
        ];
                ...
                case 'section_html':
                    if (isset($params['uid']) && $params['uid']>0) {
                        ...
                        if ($readlog = $this->getUserRecentReadLog($params['uid'], $params['cid']))
                        {
                            ...
                            unset($params['uid']);
                        } elseif (isset($params['section_num'])) {
                            // 有章节就跳转对应小说章节
                        } else {
                            // 没有阅读记录就跳转首页
                            $url = $pages['index'];
    ...
    //腾讯推广信息获取
    public function TxGetInfo(){
        $data = request()->input();
        $result = Log::info('tx data', ['data' => $data]);
        dd($result);
    }
