...
class OfficialAccountService extends Service {
    ...
    protected $token = 'xiaoshuotoken';
    protected $aes_key = 'wIdIjelOQsBphUa8j3fRSj2JAO16UPsRs4hwWyYtTSl';
    ...
    public function wechatEvent($request) {
        /*         $content = [
             "touser"=>"ohvvM0SHsCo_2W-t0vSb_lQdvGuw",
             "msgtype"=>"text",
             "text"=> ["content"=>"Hello World"]
         ];

         $content = json_encode($content);
         $res = $this->SendCustomMsg('30',$content);//发送关注欢迎
         var_dump($res);
         exit();*/
    ...
    private function qrcodeSend($plat_wechat, $message, $novel_id=0, $section=0) {
        ...
        if (!$user) {
            if (isset($wechat_config['search_sub']['switch']) && intval($wechat_config['search_sub']['switch'])==1) {
                ...
                //$message2 = [];
                //$message2['Content'] = 2;
                //$message2['FromUserName'] = -1;
                //return $this->operateText($message2);
     ...
     */
    private function noUserPush($host, $plat_wechat, $dtype=2, $uid = -1,$message) {
        $novels = $this->novels->model->where('status', '>','0')->where('title', 'like', '%'.$message.'%')->orderBy('week_read_num', 'desc')->limit(4)->select(['id', 'title', 'img','desc'])->get();
        if($novels->count()){
            if($novels->count() == 1){
                foreach ($novels as $novel) {
                    $item = [
    ...
    private function searchSubPushMsg($host, $plat_wechat, $conf, $uid = -1) {
        ...
        $url = $host . route("jumpto", ['route'=>'section', 'novel_id'=>$conf['nid'][$i], 'section_num'=>0, 'section'=>0, 'customer_id'=>$plat_wechat['customer_id'], 'dtype'=>2, /*'uid'=>$uid,*/ 'tologin'=>1, 'start'=>$conf['snum'][$i]], false);
        $str .= "👉 <a href='{$url}'>{$conf['title'][$i]}</a>\r\n\r\n";
        //$str .= "👇 为您推荐\r\n\r\n";
        $i++;
        while ($i<count($conf['nid'])) {
            if($conf['snum'][$i]){
                $url = $host . route("jumpto", ['route'=>'section', 'novel_id'=>$conf['nid'][$i], 'section_num'=>$conf['snum'][$i], 'section'=>$conf['snum'][$i], 'customer_id'=>$plat_wechat['customer_id'], 'dtype'=>2, /*'uid'=>$uid,*/ 'tologin'=>1,'start'=>$conf['snum'][$i]], false);
                $str .= "👉 <a href='{$url}'>{$conf['title'][$i]}</a> \r\n\r\n";
            }
            $i++;
        }
    ...
    private function operateText($message) {
        ...
        if (!$keyword_info) {
            ...
            $user = $this->users->FindByOpenid($message['FromUserName'], ['name', 'id', 'first_account']);
            $uid = $user ? ($user['first_account'] > 0 ? $user['first_account'] : $user['id']) : -1;
            return $this->noUserPush($host, $wechat, 2, $uid,$message['Content']);
        }
    ...
    public function test(){
        echo 1111;
        //$this->SendCustomMsg(30,'欢迎关注');//发送关注欢迎
    }
}
