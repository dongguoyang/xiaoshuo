...
class PayWechatRepository extends Repository {
     ...
     * @param bool $return
     */
	public function initWechat($type, $return = false, $customer_id = 0 ,$id = 0) {
	    if($customer_id){
            $info = $this->model->where('status', 1)->where('type', $type)->where('customer_id',$customer_id)->select($this->model->fields)->first();
            if(empty($info)){
                $info = $this->model->where('status', 1)->where('type', $type)->select($this->model->fields)->first();
            }
	    }else if($id){
	        $info = $this->model->where('id', $id)->select($this->model->fields)->first();
	    }else{
	        $info = $this->model->where('status', 1)->where('type', $type)->select($this->model->fields)->first();
	    }
        if (!$info) {
