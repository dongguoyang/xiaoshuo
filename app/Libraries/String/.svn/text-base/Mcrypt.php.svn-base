...
class Mcrypt {
	...
	/**
	 * 加密Url
	 * @param string $url 需要加密 uri 的完整url地址
	 * @param string $key 加密使用的秘钥
	 * @param string $end_str 结尾的字符串 例（.html, .htm, .asp, .php）
	 * @return string $secret_uri 需要加密 uri 的完整url地址
	 */
	public function mcryptUrl($url, $key = 'wechat', $end_str = '.htm') {
		// 检查 url 是否被转码过，转码过的需要解码
		if (strpos($url, '%3f') || strpos($url, '%26') || strpos($url, '%3d')) {
			$url = urldecode($url);
		}
		// 解析 url 信息，拼接完整的 uri
		$urls = parse_url($url);
		...
		// 生成加密的字符串
		$uri = $this->encode($uri, $key);
		// 拼接完整的 url
		$secret_url = $urls['scheme'] . '://' . $urls['host'] . '/' . $uri . $end_str;
