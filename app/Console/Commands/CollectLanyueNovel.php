<?php
/**
 * 熊总的书城 公众号《蓝月看书》 小说采集的命令
 *
 * 运行命令
 * php artisan novel:collect-lanyue-novel 1
 *
"都市"    => 3,
"历史"    => 5,
"总裁"    => 11,
...
use App\Logics\Repositories\src\Novel2Repository;
use App\Logics\Repositories\src\NovelRepository;
use App\Logics\Repositories\src\NovelSection2Repository;
...
class CollectLanyueNovel extends Command
{
    use RedisCacheTrait;
    /**
     ...
     *
     */
    protected $signature = 'novel:collect-lanyue-novel {type} {cookie}';
     ...
     * @var string
     */
    protected $description = '蓝月看书.com 小说采集程序；order is "novel:collect-lanyue-novel {type} {urlencode(cookie)}"';

    protected $cookie = 'PHPSESSID=osiod8f6qgfpdj7bbppjofko4h; Hm_lvt_fcd37298453821c3bc8b266221db88ac=1575882958,1575885839; Hm_lvt_84bbc585ea2e33b0c5a894bab2c53ab4=1575882932,1575885839,1575886617,1575886634; user=%7B%22nickname%22%3A%22%5Cu662f%5Cu54e6%22%2C%22img%22%3A%22http%3A%5C%2F%5C%2Fthirdwx.qlogo.cn%5C%2Fmmopen%5C%2Fvi_32%5C%2FDYAIOgq83eqoicKy5qiboP1kZJdhOyibBDghj29c1DGNuGkRg7LS3cIDZu5VLwmt2M4Y2z1HOKHUnOrXmibuQyuw3g%5C%2F132%22%2C%22open_id%22%3A%22oH3xe1JpIstLCIYXTUg4YO1tyZQk%22%2C%22sex%22%3A%221%22%2C%22domain%22%3A%22lyksrz.chuanyistar.com%22%2C%22addtime%22%3A%221576058638%22%2C%22open_id_pre%22%3A%22oH3xe1%22%7D; suid=1108881; Hm_lpvt_84bbc585ea2e33b0c5a894bab2c53ab4=1575945002; Hm_lpvt_fcd37298453821c3bc8b266221db88ac=1575945002';
    protected $novelTypes;  // 小说分类信息
    protected $sexes;  // 小说性别分类
    protected $serialStatus; // 连载状态
    protected $schemeHose;  // 协议和域名
    protected $disk;    // 使用的网络存储盘
        ...
        while (true) {
            ...
            foreach ($typeListLinks as $v) {
                try {
                    ...
                    dump(
                        date('Y-m-d H:i:s'),
                        'novel = ' . $novel['id'] . ' 抓取章节数据完成//异常 section-num = '.$section,
                        $msg,
                        ' === url === ' . $v,
                        '--------------------------------------------------'
                    );
                    ...
                    Log::info('lanyue-spider-info: ' . $detail['title'] . '_' . $type_id . '___' . $msg);
                } catch (\Exception $e) {
                    dump(
                        date('Y-m-d H:i:s'),
                        $e->getMessage(),
    ...
    private function initConfig() {
        ...
        $this->schemeHose = 'http://lyks.pengyu91.cn';
    }
    /**
     * 获取分类信息数据
     */
    private function classifyData() {
        $url = $this->schemeHose . '/Home/Category/index';
        $html = $this->cookieCurl($url);

        preg_match('/classify-box[.\s\S]*?(<ul[.\s\S]*?<\/ul>)/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
            throw new \Exception('分类信息字符串异常匹配！', 2000);
        }
        preg_match_all('/val="(\d*?)".*?>(.*?)<\/a>/', $match[1], $match);
    ...
    private function typesListLink($tid, $p) {
        $url = $this->schemeHose . '/Category/ajaxGetNovelList';
        $data = [
            'tid' => $tid,
            'vip' => '',
            'end' => '',
        ...
        $html = $this->cookieCurl($url, [], $data);

        $html = json_decode(trim($html), 1);
        if (!isset($html['data']) || !$html['data'] || !is_string($html['data'])) {
            throw new \Exception('分类列表数据异常！', 2000);
        }

        preg_match_all('/href="([.\s\S]*?)"/', $html['data'], $match);
    ...
    private function novelPageHtml($url) {
        ...
        preg_match('/<a class="btn-see-more" href="(.*?)"/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
            throw new \Exception('章节目录匹配失败！', 2000);
        }
        $mulu = $match[1];

        $rel['spider_url'] = $this->firstSectionUrl($html); // 获取第一章阅读链接

        preg_match('/<div class="entry"><img src="(.*?)" class="avatar" id="avatar"/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
        ...
        preg_match('/<div class="body">([.\s\S]*?)<div class="novel-catalog block">/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
            throw new \Exception('novel describe info 异常匹配！', 2000);
        }
        $head = $match[1];

        preg_match('/<h1>(.*?)<\/h1>/', $head, $match);
        if (!isset($match[1]) || !$match[1]) {
            throw new \Exception('标题异常匹配！', 2000);
        }
        $rel['title'] = trim($match[1]);

        preg_match('/<p>作者：(.*?)<\/p>/', $head, $match);
        if (!isset($match[1]) || !$match[1]) {
        ...
        preg_match('/<a href="javascript:void\(0\);" class="red">([.\s\S]*?)<\/a>/', $head, $match);
        if (!isset($match[1]) || !$match[1]) {
        ...
        preg_match('/<i class="green">([.\s\S]*?)<\/i>/', $head, $match);
        if (!isset($match[1]) || !$match[1]) {
            throw new \Exception('连载状态异常匹配！', 2000);
        }
        $rel['serial_status'] = $this->serialStatus[trim($match[1])];

        preg_match('/<p class="gray" style="color:#ff730b;">([.\s\S]*?)万人正在追/', $head, $match);
        if (!isset($match[1]) || !$match[1]) {
            // throw new \Exception('追书人数异常匹配！', 2000);
            $rel['hot_num'] = 5;
        } else {
            $rel['hot_num'] = trim($match[1]);
        }
        ...
        /* preg_match('/<p class="bh_book_title_hide" onclick="bh_book_title_hide\(\);" style="display: none;">([.\s\S]*?)<\/p>/', $head, $match);
         if (!isset($match[1]) || !$match[1]) {
             throw new \Exception('简介异常匹配！', 2000);
         }
         $rel['desc'] = substr(trim(strip_tags($match[1])), 0, 252);*/
        $rel['desc'] = $this->matchDesc($head);
    ...
    private function firstSectionUrl($html) {
        /*preg_match('/<a href="(\/Home[.\s\S]*?)"/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
            throw new \Exception('阅读链接异常匹配！', 2000);
        }*/

        preg_match('/<a class="weui_cell" href="(\/Home[.\s\S]*?)"/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
    ...
    private function addNovelInfo($detail) {
        if ($had = $this->novels->findByMap([
        ...
        ], ['id', 'sections', 'spider_url', 'serial_status'])) {
            ...
            try{
                ...
                $detail['status'] = 0;
                $detail['origin_web'] = $this->novelWeb;
     ...
     * 跳转到这部小说之前采集的最后一章
     */
    private function jumpToSpiderEnd($mulu, $title, $page = 1, $id = 0) {
        if (!$id) {
            if (!$mulu) {
                throw new \Exception('目录异常；章节链接不能获取', 2000);
            }
            if (strpos($mulu, '/novel_id/')) {
                $arr = explode('/novel_id/', $mulu);
            } else {
                $arr = explode('/id/', $mulu);
            }
            $id = $arr[1];
        }
        $mulu = '/novel/ajaxcataloglist';
        $params = ['id'=> $id, 'od'=>'', 'start'=>$page, 'cid'=>0];
        $html = $this->cookieCurl($this->schemeHose . $mulu, [], $params);
        if (!$html) {
        ...
        if (is_array($arr)) {
            foreach ($arr as $v) {
                if (strpos($title, $v['chapter']) !== false && strpos($title, $v['title']) !== false) {
                    return '/Home/Novel/show/id/'. $v['id'] .'/novel_id/' . $id;
                }
            }
            return $this->jumpToSpiderEnd($mulu, $title, ++$page, $id);
        } else {
            throw new \Exception('章节链接目录信息获取失败', 2000);
        }
    }
    /**
     * 抓取并添加章节信息
    ...
    private function spiderSectionInfo($novel, $mulu) {
        ...
        while (true) {
            ...
            $html0 = $this->cookieCurl($url);
            preg_match('/<\!\-\- 文章主题内容 \-\->([.\s\S]*?)<p class="collect">/', $html0, $match);
            if (!isset($match[1]) || !$match[1]) {
                return [$section['num'], $novel['spider_url'], '章节内容 html 获取失败' . $url];
            }
            $html = $match[1];
            ...
            if ($section_had) {
                ...
                if ($had = $this->novelSections->findByMap([
                    ...
                    if ($had) {
                        // 该章节不存在；或者改章节是最后章节；则获取文章下一页
                        $match[1] = $this->matchNextSectionUrl($html0);
                        if (!isset($match[1]) || !$match[1]) {
            ...
            preg_match('/<p class="article\-desc"[.\s\S]*?(<p>[.\s\S]*?)<\/div>/', $html, $match);
            if (!isset($match[1]) || !$match[1]) {
            ...
            if ($section['title'] != $old_title) {
                ...
                $this->spidered++;
                //sleep(mt_rand(5, 15)); // 抓取一章后休息一会，模拟阅读；防止被标记为机器人
    ...
    private function matchDesc($html) {
        preg_match('/<p class="bh_book_title_hide" onclick="bh_book_title_hide\(\);" style="display: none;">([.\s\S]*?)<\/p>/', $html, $match);
        $str = '';
    ...
    private function matchNextSectionUrl($html) {
        preg_match('/<li id="next\_page">.*?href=[\'\"](.*?)[\'\"]/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
            return null;
