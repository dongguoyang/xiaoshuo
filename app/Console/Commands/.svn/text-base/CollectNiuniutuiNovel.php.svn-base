 ...
 *
 * 运行命令
 * php artisan novel:collect-niuniutui-novel urlencode(cookie)  // 采集全部
 * php artisan novel:collect-niuniutui-novel urlencode(cookie) --type=玄幻  // 采集玄幻分类
 * php artisan novel:collect-niuniutui-novel urlencode(cookie) --link=http://zjj.muoqe.cn/r/ijkAWcHYw9jBm9aNlNlx,,http://zjj.muoqe.cn/r/ghiB2FTN10xxUzoIZFZvx  // 采集指定link
 *
...
class CollectNiuniutuiNovel extends Command
{
    use RedisCacheTrait;
    /**
     ...
     *
     */
    protected $signature = 'novel:collect-niuniutui-novel {cookie} {--H|host= : 指定采集的域名} {--T|type= : 中文类型名称/all} {--L|link= : 单独采集一部小说的链接地址，多个链接用 ,, 两个英文逗号进行分隔}';
     ...
     * @var string
     */
    protected $description = 'niuniutui.com 小说采集程序；order is "novel:collect-niuniutui-novel {cookie} {--T|type= : 中文类型名称/all} {--L|link= : 单独采集一部小说的链接地址，多个链接用 ,, 两个英文逗号进行分隔}"';
        ...
        while (true) {
            ...
            foreach ($typeListLinks as $v) {
                try {
                    ...
                    if ($section > 20 && $novel['status'] == 0) $novel_up['status'] = 1;
    ...
    private function initConfig() {
        ...
        $this->schemeHose = 'https://wx0e5baad403af8147.liangjinzhurou.cn';
    }
    /**
     * 获取分类信息数据
     */
    ...
    private function jumpToSpiderEnd($mulu, $title) {
        if (strpos($mulu, 'http') === false) {
            $mulu = $this->schemeHose . $mulu;
        }
        $html = $this->cookieCurl($mulu);
    ...
    private function spiderSectionInfo($novel, $mulu) {
        ...
        //dd($this->novels->toArr($novel), $this->novels->toArr($section));
        if ($section) {
            $section = $this->novelSections->toArr($section);
            if ($mulu) $novel['spider_url'] = $this->jumpToSpiderEnd($mulu, $section['title']);
        } else {
            $section['num'] = 0;
        ...
        while (true) {
            ...
            preg_match('/<div class="content" id="idcontent".*?>([.\s\S]*?)<\/div>/', $html, $match);
            if (!isset($match[1]) || !$match[1]) {
            ...
            if ($section['title'] != $old_title) {
                ...
                $this->spidered++;
                //sleep(mt_rand(5, 12)); // 抓取一章后休息一会，模拟阅读；防止被标记为机器人
                sleep(mt_rand(10, 30)); // 抓取一章后休息一会，模拟阅读；防止被标记为机器人
                if ($this->spidered > 200) { // 看了 200 章就去上个厕所回来再看
                    $this->spidered = 0;
                    //sleep(mt_rand(1, 5) * 60);
                    sleep(mt_rand(3, 10) * 60);
                }
            }
            if (!isset($match[1]) || !$match[1]) {
     ...
     * 抓取指定链接的小说
     */
    private function spiderANovel($link) {
        $parUrl = parse_url($link);
        if (!isset($parUrl['scheme']) || !isset($parUrl['host'])) {
            throw new \Exception('链接异常，不能抓取！', 2000);
        }
        $this->schemeHose = $parUrl['scheme'] . '://' . $parUrl['host'];

        list($link, $mulu) = $this->aNovelSection1Link($link); // 获取第一章链接地址
        $html = $this->cookieCurl($link);
        preg_match('/<div class="content" id="idcontent".*?>([.\s\S]*?)<\/div>/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
            throw new \Exception('章节正文内容获取失败！', 2001);
        }
        $content = TrimAll(strip_tags($match[1]));

        $novel['title'] = '单爪.' . mb_substr($content, 0, 15);
        $novel['spider_url'] = $parUrl['path'];
        $novel['img'] = $this->disk=='local' ? '/storage/lm-novelspider/lanyue/img/3/20191230/175110_ze4iqvgyupm690zqau.png' : 'https://ccccccc1111111.oss-cn-hangzhou.aliyuncs.com/lm-novelspider/baozou/img/6/20200114/165442_ou3o2nknbr91m792ep.jpg';
        $novel['desc'] = mb_substr($content, 0, 50);
        $novel['suitable_sex'] = mt_rand(1, 2);
        $novel['hot_num'] = mt_rand(1, 10);
        $novel['word_count'] = mt_rand(10, 100);
        $novel['type_ids'] = current($this->novelTypes);
        $novel['serial_status'] = 1;
        $novel['status'] = 0;
        $novel['author_name'] = '宝宝疯';
        $novel['author_id'] = 1;
        dump($novel);
        ...
        if ($had = $this->novels->findByMap([
            ...
            dump('该小说已经采集过了！', $this->novels->toArr($had));
            $novel = $had;
            // throw new \Exception('该小说已经采集过！', 2000);
        } else {
            $novel = $this->novels->create($novel);
        }
        list($section, $spider_url, $msg) = $this->spiderSectionInfo($novel, $mulu);// 抓取章节数据；抛出异常就不能更新总章节数
        // 更新novel数据
        $novel_up = ['sections'=>$section, 'spider_url'=>$spider_url];
        if ($section > 20 && $novel['status'] == 0) $novel_up['status'] = 2;
        $this->novels->update($novel_up, $novel['id']);// 更新小说总章节数据
     ...
     * 获取第一章链接地址
     */
    private function aNovelSection1Link($link) {
        if (strpos($link, 'http') === false) {
            $link = $this->schemeHose . $link;
        }
        // 获取目录页面地址
        $html = $this->cookieCurl($link);
        preg_match('/背景颜色.*?<li.*?<a href="([.\s\S]*?)"/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
            throw new \Exception('目录地址获取失败！', 2000);
        }
        $mulu = $match[1];
        ...
        // 获取第一章地址
        $html = $this->cookieCurl($mulu);
        preg_match('/<div id="tab\_conbox1">.*?<a .*?data-url="([.\s\S]*?)"/', $html, $match);
        if (!isset($match[1]) || !$match[1]) {
            throw new \Exception('第一章链接地址获取失败！', 2000);
        }
        $link = $match[1];
