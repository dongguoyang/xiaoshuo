...
class CollectBiqugeOneNovel extends Command
{
    use CollectTrait;
    /**
     ...
     *
     */
    protected $signature = 'novel:collect-biqugeone-wap {cookie} {--H|host= : 指定采集的域名} {--L|link= : 单独采集一部小说的链接地址，多个链接用 ,, 两个英文逗号进行分隔}';
    /**
        ...
        if ($link && strpos(trim($link), 'http') === 0) {// 抓取一本小说
            ...
            try {
                ...
                foreach ($links as $link) {
                    ...
                    try {
                        $book_id = intval(explode('book_', $link)[1]);
